...
function createProject({ name, rootPath, targetType = 'local-windows', serverId = null, linuxLayout, releaseRootPath }) {
  const projectInput = validateProjectInput({ name, rootPath, targetType, serverId, linuxLayout, releaseRootPath });
  const data = db.read();
 ...
 */
function updateProject({ projectId, name, rootPath, targetType = 'local-windows', serverId = null, linuxLayout, releaseRootPath }) {
  const projectInput = validateProjectInput({ name, rootPath, targetType, serverId, linuxLayout, releaseRootPath });
  const data = db.read();
...
function validateProjectInput({ name, rootPath, targetType, serverId, linuxLayout, releaseRootPath }) {
...
function syncRemoteReleases({ projectId, remoteReleases, activeFolderName }) {
  ...
  data.projects[projectIndex].activeReleaseId = activeRelease?.id || null;
  db.write(data);
